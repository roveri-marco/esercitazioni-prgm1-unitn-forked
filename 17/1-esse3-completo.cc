...
Studente* genera_studente(char  nome[],
                          ...
                          int   matricola,
                          float media);
bool riempi_database(Studente **database,
                     char       file[],
                     int        dim);
int  cerca_matricola(Studente **database,
                     int        matricola,
                     int        dim);
int  cerca_nome_e_cognome(Studente **database,
                          char       nome[],
                          char       cognome[],
                          int        dim);
int studente_top_media(Studente **database,
                       int        dim);
    ...
    while (azione != 'e') {
        ...
        switch (azione) {
          case 'i':
          {
              if (inseriti ==
                  MAX_DATABASE)cout <<
                      "[ESSE3++] Raggiunta la dimensione massima del database!"
                                    <<
                      endl;
              else {
                  ...
                  Studente *s_tmp = genera_studente(nome,
                                                    cognome,
                                                    matricola,
                                                    media);
          ...
          case 'l':
          {
              int indice;
              cout <<
                  "[ESSE3++] Inserisci l'indice dell'utente che vuoi leggere: ";
              cin >> indice;

              if (indice >=
                  inseriti)cout <<
                      "[ESSE3++] L'utente con questo indice non esiste!" <<
                    endl;
              else stampa_studente(database[indice]);
              break;
          }
          case 'f':
          {
          ...
          case 'm':
          {
              ...
              if (indice != -1)stampa_studente(database[indice]);
              else cout << "[ESSE3++] Studente non trovato!" << endl;
              break;
          }
          case 'n':
          {
              ...
              cin >> nome >> cognome;
              int indice = cerca_nome_e_cognome(database,
                                                nome,
                                                cognome,
                                                inseriti);
...
int cerca_nome_e_cognome(Studente **database,
    ...
    for (int i = 0; (i < dim) && (indice == -1); i++) {
        if ((strcmp(database[i]->nome,
                    nome) == 0) && (strcmp(database[i]->cognome,
                                           cognome) == 0)) {
            indice = i;
        }
    }
